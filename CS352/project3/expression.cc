...
// Optimization
AST_Exp * AST_Exp::optimizeImmediateValues() {
    switch (expression) {
        case Expression::Binary:
            return binaryOp->optimizeImmediateValues();
        case Expression::Unary:
            return unaryOp->optimizeImmediateValues();
        case Expression::Parens: {
            if (parens->exp->expression == INT_LITERAL || parens->exp->expression == EBool)
                return parens->exp;
            else
                return parens->exp->optimizeImmediateValues();
        }
        case Expression::Index:
        case Expression::Length:
        case Expression::IndexLength:
...
AST_Exp * Exp_BinaryOp::optimizeImmediateValues() {
    //optmize left and right
    if (left->expression != INT_LITERAL && left->expression != EBool) {
        //optimize left
        AST_Exp * newLeft = left->optimizeImmediateValues();
        if (newLeft != NULL)
            left = newLeft;
        else
            return NULL;
    }
    if (right->expression != INT_LITERAL && right->expression != EBool) {
        //optimize right
        AST_Exp * newRight = right->optimizeImmediateValues();
        if (newRight != NULL)
            right = newRight;
        else
            return NULL;
    ...
    if (op.compare("&&") == 0 || op.compare("||") == 0) {
        if (op.compare("&&") == 0)
            return new AST_Exp(left->boolLiteral && right->boolLiteral);
        else
            return new AST_Exp(left->boolLiteral || right->boolLiteral);
    }
    //boolean, int -> boolean operations
    else if (op.compare("==") == 0 || op.compare("!=") == 0) {
        if (left->expression == EBool) {
            if (op.compare("==") == 0)
                return new AST_Exp(left->boolLiteral == right->boolLiteral);
            else
                return new AST_Exp(left->boolLiteral != right->boolLiteral);
            ...
            if (op.compare("==") == 0)
                return new AST_Exp(left->intLiteral == right->intLiteral);
            else
                return new AST_Exp(left->intLiteral != right->intLiteral);
        }
    }
    ...
    //int -> int operations
        ...
        cout << "Unable to optimize binary op, unknown operator" << endl;
        return NULL;
    }
}
AST_Exp * Exp_UnaryOp::optimizeImmediateValues() {
    //optimize expression
    if (exp->expression != INT_LITERAL && exp->expression != EBool) {
        AST_Exp * optimizedExp = exp->optimizeImmediateValues();
    ...
    if (exp->expression == INT_LITERAL) {
        //evaluate
        if (op == '+') {
            return new AST_Exp(exp->intLiteral);
        }
        else {
            return new AST_Exp(-1 * exp->intLiteral);
        }
    }
    else if (exp->expression == EBool) {
        //evaluate
        return new AST_Exp(!exp->boolLiteral);
    }
    return NULL;
}
