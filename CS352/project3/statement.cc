...
//Optimizing
void AST_Statement::optimizeImmediateValues() {
    switch (type) {
        case StatementType::List:{
            for (vector<AST_Statement*>::iterator stmt = statementList->statements.begin();
            stmt != statementList->statements.end(); stmt++) {
        ...
        case StatementType::Print: {
            if (printStatement->printExp) {
                AST_Exp * newExp = printStatement->exp->optimizeImmediateValues();
                if (newExp != NULL)
                    printStatement->exp = newExp;
            }
            break;
        }
        case StatementType::SBool: {
            AST_Exp * newExp = boolStatement->exp->optimizeImmediateValues();
            if (newExp != NULL)
                boolStatement->exp = newExp;
            boolStatement->statement->optimizeImmediateValues();
            if (boolStatement->ifStatement)
                boolStatement->elseStatement->optimizeImmediateValues();
            break;
        }
        case StatementType::Assign: {
            AST_Exp * newExp = assignStatement->exp->optimizeImmediateValues();
            if (newExp != NULL)
                assignStatement->exp = newExp;
            break;
        }
        case StatementType::Return: {
            AST_Exp * newExp = returnStatement->exp->optimizeImmediateValues();
            if (newExp != NULL)
                returnStatement->exp = newExp;
            break;
        }
        case StatementType::Empty:
            break;
