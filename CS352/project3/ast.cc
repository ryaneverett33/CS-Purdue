...
void Program::optimize() {
    ...
    // main class
    MainClass->statement->optimizeImmediateValues();
    // class->method
...
// get the depth of the index object
...
// flatten an index to a list of expressions in correct order
AST_Exp ** AST_Index::flatten() {
    AST_Exp ** array = new AST_Exp*[depth];
    if (!recursive) {
        array[0] = exp;
        return array;
    }
    ...
    switch (type) {
        case NewID: {
           ...
           TableRow * classRow = globalTable->getSymbolInfo(id);
            ...
            resolvedType = new ReturnType(id, 0);
            typeSet = true;
            return resolvedType;
        }
        case OID: {
        ...
        case This: {
            ...
            resolvedType =  new ReturnType(className, 0);
            typeSet = true;
            return resolvedType;
        }
        case NewPrimeType: {
            ...
            switch (newPrimeType->primeType->type) {
                case Int:
                    resolvedType =  new ReturnType(PINT, indexType->arrayDepth);
                    typeSet = true;
                    return resolvedType;
                case TBool:
                    resolvedType = new ReturnType(PBOOL, indexType->arrayDepth);
                    typeSet = true;
                    return resolvedType;
                case TID:
                    resolvedType = new ReturnType(newPrimeType->primeType->IDValue, indexType->arrayDepth);
                    typeSet = true;
                    return resolvedType;
            }
        }
...
void Program::resolveProgramName(char * name) {
    if (name == NULL)
        return;
    // iterate through name until NULL or '.'
    string tmpName(name);
...
// Return allocation size for class
int AST_ClassDecl::getSize() {
    int count = varDeclList->count * 4;
    if (!simple) {
        cout << "getSize doesn't include definitions for extension classes" << endl;
        TableRow * row = symbolTable->getSymbolInfo(extendsId);
        if (row == NULL) {
            cout << "Symbol table lookup failed for " << extendsId << " when getting class size" << endl;
        }
        else if (row->symbolType != SCLASS) {
            cout << "Symbol table lookup for " << extendsId << " returned incorrect type for class size" << endl;
        }
        else {
            AST_ClassDecl * extendClass = row->node->astClassDecl;
            count += extendClass->getSize();
        }
    }
    return count; 
}