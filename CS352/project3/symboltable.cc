...
bool SymbolTable::addClass(string name, AST_Node * node, SymbolTable * table, bool mainClass) {
    ...
    int classSize = mainClass ? 0 : node->astClassDecl->getSize();
    row->addClassInfo(new ClassInfo(mainClass, classSize));
    rows.push_back(row);
    symbolCount++;
    return true;
}
...
void ClassInfo::printDebug() {
    cout << "\tMain Class: " << mainClass << ", size: " << classSize << endl;
}
...
// Merges a symbol table with its inherited table
SymbolTable * SymbolTable::resolveInheritedTable() {
    if (this->inheritedTable == NULL)
        return this;
    SymbolTable * copyTable = new SymbolTable(*this);
    for (int i = 0; i < inheritedTable->rows.size(); i++) {
        TableRow * row = inheritedTable->rows[i];
        TableRow * copyRow = new TableRow(*row);
        copyTable->rows.push_back(copyRow);
        copyTable->symbolCount++;
    }
    return copyTable;
}